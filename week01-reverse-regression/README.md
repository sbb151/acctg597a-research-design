...
4. **The design was institutionalized, not corrected.** Basu (1997) — 4,200+ citations — builds conditional conservatism on the same earnings-on-returns direction plus truncation on the sign of the regressor. In a simulated world with perfectly symmetric earnings timeliness and right-skewed non-earnings return noise, the Basu regression "finds" conservatism in every sample (mean spurious b₃ = 0.21, mean t = 5.1, 99.9% rejection rate at N = 700); with symmetric noise the test rejects 3.8% of the time, consistent with nominal size.
...
- `figures/fig3_spurious_basu.png` — sampling distribution of the spurious Basu coefficient (true b₃ = 0).
